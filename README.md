...
3. **开始阅读** — 点击图书 → 自动按章节分页，左右点击翻页，中间点击菜单
...
| `mode` | 是 | `chapter` 或 `scroll` |
| `book` | 否 | 书籍信息（scroll 模式可不填） |
| `book.title` | 否 | 书名选择器 |
| `book.author` | 否 | 作者选择器 |
| `book.cover` | 否 | 封面图选择器 |
| `book.description` | 否 | 简介选择器 |
| `chapterList` | chapter模式必填 | 章节目录提取 |
...
- 仅支持**服务端渲染**的 HTML 页面（SPA/JS 动态渲染网站不支持，如 bqg661.cc）
- 编码回退链：utf-8 → gbk → latin1
- 网络请求自动重试 3 次
